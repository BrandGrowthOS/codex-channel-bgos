...
This release is HELD BACK from `latest`, which inverts that order for 0.10.1 only. The daemon now offers a thirty minute approval hold and relies on the BGOS backend clamping that offer to the owner's per agent choice (see the RELEASE ORDER comment above `APPROVAL_HOLD_SECONDS` in `src/interactions.ts`). So the publish workflow sends this version to npm under the dist tag `next`, named in `HELD_FROM_LATEST` in `.github/workflows/publish.yml`, and nothing installs it by default. Once that backend and its migration are deployed, promote it by hand and drop the version from that list:
