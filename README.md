...
| Stop and resume | An owner Stop (the app's Stop button or `/stop`) cancels the turn, posts "Stopped." and pauses the chat's open mission with the reason "Stopped by you" instead of failing it; the owner's next message in that chat, the app's Resume included, resumes it. In a Keep working chat the Stop also holds the goal, during a continuation turn the runtime started on its own too, and that message gives it back only if the goal was running when the Stop came; a goal held at its turn cap, stopped for lack of progress, or paused with `/goal pause`, before the Stop or between the Stop and that message, stays held, a daemon restart before that message included, and the message runs as an ordinary turn. `/new`, a daemon shutdown and a revoked pairing still end the mission. Declared as `stop_pauses_mission` |
