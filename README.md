...
| Generated images | In a chat turn, a picture Codex makes with its image generation tool posts itself as an ordinary image when the turn finishes, first in the reply (ahead of any card, question or buttons), with `Prompt: <revised prompt>` as its caption, so it also shows in the gallery. Posted from the decoded bytes, never from a file, and never mid turn (a post mid turn would mark the agent done for the rest of it), except just before a plan card the turn raises. A picture refused by the image limit posts one plain line naming the reset time; one that was made but could not be shown says so in one plain line. After a Stop the finished pictures still post, after the stop line, without holding anything up. A meeting or a voice task does not post pictures; the agent is told so |
